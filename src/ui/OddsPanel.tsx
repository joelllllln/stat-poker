...
import { liveOpponentRanges, modelledWidth } from '../equity/opponent'
...
export function OddsPanel({ state, heroSeat }: { state: HandState; heroSeat: number }) {
  ...
  const analysis = useMemo(() => {
    if (!hero.holeCards || hero.status === 'folded') return null
    const opponents = liveOpponentRanges(state, heroSeat)
    if (opponents.length === 0) return null

    try {
      const equity = handEquity(hero.holeCards, opponents, state.board, {
        rng: new Rng(state.actions.length * 7919 + 13),
        iterations: 8_000,
      })
      const widths = state.seats
        .filter((s) => s.index !== heroSeat && s.status !== 'folded')
        .map((s) => ({ name: s.name, width: modelledWidth(state, s.index) }))
      return { equity, widths }
    } catch {
      return null
    }
  }, [state, heroSeat, hero.holeCards, hero.status])

  if (!analysis) return null

  const { equity, widths } = analysis
        ...
        <>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            <Stat
              ...
              hint={equity.exact ? 'exact' : `±${(equity.errorMargin * 100).toFixed(1)}`}
            />
            <Stat
              label="Need to call"
