...
export function SolveRiver({ state, heroSeat }: { state: HandState; heroSeat: number }) {
  ...
  return (
    <div className="rounded-lg border border-[color:var(--color-royal)]/50 bg-[color:var(--color-royal-deep)]/60 px-3 py-2">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <button
          ...
          disabled={pending}
          className="rounded bg-[color:var(--color-royal)] px-2.5 py-1 text-xs hover:bg-[color:var(--color-royal-bright)]/30 disabled:opacity-40"
        >
          {pending ? 'Solving…' : result ? 'Solve again' : 'Solve'}
        </button>
      </div>

      {error && <div className="mt-1 text-xs text-[color:var(--color-oxblood-bright)]">{error}</div>}

      {result && (
        <div className="mt-2 space-y-1.5">
          <div className="text-xs text-[color:var(--color-bone)]">
          ...
          {result.actions
            ...
            .map((action) => (
              <div key={action.label} className="flex items-center gap-2 text-xs">
                <span className="w-20 shrink-0 text-[color:var(--color-royal-bright)]">{action.label}</span>
                <div className="h-2 flex-1 overflow-hidden rounded bg-[color:var(--color-royal-deep)]">
                  <div
                    className="h-full bg-[color:var(--color-royal)]"
                    style={{ width: `${action.frequency * 100}%` }}
                  />
                </div>
                <span className="w-12 shrink-0 text-right font-mono text-[color:var(--color-royal-bright)]">
                  {(action.frequency * 100).toFixed(1)}%
                </span>
              </div>
            ))}
          <div className="text-[10px] text-[color:var(--color-royal-bright)]/50">
